...
import numpy as np
from models import Actor, Critic2
...
class TD3Agent:
	def __init__(self, num_states, num_actions, num_goals, action_bounds, mem_size, env, k_future, batch_size, 
		...
		self.critic        = Critic2(critic_input_shape)

		self.sync_networks(self.actor.network)
		self.sync_networks(self.critic.network1)
		self.sync_networks(self.critic.network2)
		...
		# Initialse noise
		self.action_noise = OrnsteinUhlenbeckActionNoise(mu=np.zeros(num_actions), sigma=float(0.2) * np.ones(num_actions))
	...
	def train(self, num_updates, policy_freq=2):
		'''
		Note: 
		In TD3, there are 2 critic networks within each critic / critic_target objects
		The minimum value of the 2 during prediction will be selected
		'''
		cycle_actor_loss = 0
		cycle_critic_loss = 0
		...
		for it in range(num_updates):
			...
			if it % policy_freq == 0:
				...
				# Update actor loss
				cycle_actor_loss += actor_loss.numpy()

			# Critic optimization
			with tf.GradientTape(persistent=True) as tape:
				next_a = self.actor_target(next_inputs)
				temp = np.concatenate((next_inputs, next_a), axis=1)
				q_x, q_y = self.critic_target(temp)
				target_q = rewards + self.gamma * tf.math.minimum(q_x, q_y)
				temp2 = np.concatenate((inputs, actions), axis=1)
				q_eval_x, q_eval_y = self.critic(temp2)
				critic_loss = tf.reduce_mean((q_eval_x - target_q)**2) + tf.reduce_mean((q_eval_y - target_q)**2)
			q1_grads = tape.gradient(critic_loss, self.critic.network1.trainable_variables)
			q2_grads = tape.gradient(critic_loss, self.critic.network2.trainable_variables)
			
			# Sync gradients with other parallel processes
			q1_grads = self.sync_grads(q1_grads)
			q2_grads = self.sync_grads(q2_grads)
			self.critic_optimizer.apply_gradients(zip(q1_grads, self.critic.network1.trainable_variables))
			self.critic_optimizer.apply_gradients(zip(q2_grads, self.critic.network2.trainable_variables))

			# Update critic loss
			cycle_critic_loss += critic_loss.numpy()

		# Returns the values of actor_loss and critic_loss
		return actor_loss.numpy(), critic_loss.numpy(), cycle_actor_loss, cycle_critic_loss
	...
	def update_networks(self):
		...
		Critic2.soft_update_network(self.critic, self.critic_target, self.tau)
